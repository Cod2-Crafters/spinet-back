...
(
    ...
    `TAG_NAME`   VARCHAR(255) binary NOT NULL UNIQUE,
    `CHOSUNG`    VARCHAR(255) NOT NULL COMMENT '초성',
...
(
    ...
    `LOGIN_TYPE`  ENUM('BASIC','GOOGLE','KAKAO','NAVER') NOT NULL DEFAULT 'BASIC' COMMENT '로그인타입4가지',
    `SHOP_NAME`   VARCHAR(100) NOT NULL COMMENT '상점명 (젤많이쓸거)',
    ...
    `PHONE`       VARCHAR(12) DEFAULT NULL COMMENT '핸드폰번호',
    `CREATED_AT`  DATETIME(6) DEFAULT NULL COMMENT '생성일자',
...
(
    ...
    `PARENT_ID`  BIGINT(20) DEFAULT NULL COMMENT '상위 카테고리',
    `NAME`       VARCHAR(255) NOT NULL UNIQUE COMMENT '카테고리영',
